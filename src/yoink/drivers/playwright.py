...
class PlaywrightDriver(BaseDriver):
    ...
    def default_init_code(self) -> Page:
        from drivers.base import JS_SETUP_GET_EVENTS
    ...
    def code_for_resize(self, width: int, height: int) -> str:
    ...
    def resolve_xpath(self, xpath: str) -> Locator:
    ...
    def exec_code(
        ...
        for item in data:
            ...
            if action_name == "click":
                ...
                raise NoElementException("No element: " + item["action"]["args"]["value"])
            elif action_name == "failAmbiguous":
                raise AmbiguousException("Ambiguous: " + item["action"]["args"]["value"])
    ...
    def wait_for_idle(self) -> None:
        t = time.time()
        try:
            self.page.wait_for_load_state("networkidle", timeout=self.waiting_completion_timeout)
        except:
            # timeout occurred
            pass
        elapsed = time.time() - t
