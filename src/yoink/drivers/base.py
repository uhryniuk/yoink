...
class BaseDriver(ABC):
    def __init__(self, url: Optional[str], init_function: Optional[Callable[[], Any]]) -> None:
    ...
    @abstractmethod
    def resize_driver(driver, width, height) -> None:
        """
    ...
    def resolve_xpath(self, xpath) -> None:
        """
    ...
    @abstractmethod
    def code_for_execute_script(self, js_code: str) -> None:
...
class ScrollDirection(Enum):
    ...
    def get_scroll_xy(self, dimension: List[float], scroll_factor: float = 0.75) -> Tuple[int, int]:
...
def js_wrap_function_call(fn: str) -> str:
    return "(function(){" + fn + "})()"
