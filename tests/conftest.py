...
from yoink.config import Config, RateLimitConfig, ServiceConfig, WorkerConfig
...
def test_config() -> Config:
    """Minimal Config suitable for tests — 1 worker, headless, no auth."""
    return Config(
        workers=WorkerConfig(count=1, pages_per_worker=2, headless=True),
        rate_limit=RateLimitConfig(),
        service=ServiceConfig(api_key=None),
    )
...
def authed_config() -> Config:
    ...
    return Config(
        workers=WorkerConfig(count=1, pages_per_worker=2, headless=True),
        service=ServiceConfig(api_key="test-secret"),
    )
